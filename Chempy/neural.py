...
def training_data():
	...
	if 'beta_param' in a.to_optimize or 'log10_beta' in a.to_optimize:
		raise NameError('Remove beta parameter from a.to_optimize in parameter.py to continue')
	...
	r = list(tqdm.tqdm(p.imap_unordered(create_dataset, param_grid), total=len(param_grid)))#len(param_grid)))
	params = []
	norm_params = []
	abun = []
	for item in r:
	...
	np.save(directory+'training_abundances.npy', abun)
	np.save(directory+'training_norm_grid.npy',norm_params)
	np.save(directory+'training_param_grid.npy',params)
...
def neural_output_int(test_input,a,b):
	...
	# Normalize data for input into network
	if 'log10_beta' in a.to_optimize or 'beta_param' in a.to_optimize:
		norm_data = (test_input - a.p0[1:])/np.array(a.training_widths) # [1:] removes beta parameter
	else:
		norm_data = (test_input - a.p0)/np.array(a.training_widths) 
